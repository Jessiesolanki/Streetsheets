...
import { ERROR, LOADING } from '../Providers/Index';
...
export default useGeolocation = ({ dependencies = [], interval, showLoading = false } = {}) => {
    ...
    useEffect(() => {
        if (showLoading) setLoading(position == LOADING)
    }, [position])
...
const requestLocationPermission = async () => {
    try {
        ...
        if (granted === PermissionsAndroid.RESULTS.GRANTED) {
            ...
            Alert.alert('Notice', 'StreetSheets needs your location to work properly.')
...
const getCurrentPosition = () => {
    return new Promise(async (resolve, reject) => {
        ...
        if (permission == YES) Geolocation.getCurrentPosition(info => resolve(info), error => console.log(error,'errorrr'), { enableHighAccuracy: true, timeout: 30000, maximumAge: 1000 })
        else {
