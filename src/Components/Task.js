...
const Task = ({navigation,id}) => {
  ...
  const { API_CALLS, GetNoteStatus, listData,GetTaskList} = useContext(LeadContext)
...
useEffect(() => {
  getTaskListById({params: {id:id}, onSuccess: () => { setRefreshing(false);}, screenName: 'task'});  
...
const renderItem= ({item})=>{
  ...
  return (
   ...
   {listData?.data?.length > 0 ? <FlatList
   data={listData?.data} 
