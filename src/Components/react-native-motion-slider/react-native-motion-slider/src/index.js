...
class Slider extends Component {
	...
	calculateValue(posX) {
		...
		this.setState({ value });
		onDrag();
		// parseInt(value / 2)}:${parseInt(decimalStr / 1.66)
		const decimalStr = value.toString().split('.')[1];
		let a = parseInt(decimalStr / 1.66)
		let time = 10 > a ? String(a).padStart(2, '0') : a
		onValueChanged(parseInt(value / 2) + ":" + time);
	}
	...
	onPressOut() {
		...
		this.setState({
			...
			// console.log(10 > parseInt(decimalStr / 1.66), 'ooooooooooooooooooooo')
			let a = parseInt(decimalStr / 1.66)
			let time = 10 > a ? String(a).padStart(2, '0') : a
			onValueChanged(value.length == 1? '0'+parseInt(value / 2) + ":" + time:parseInt(value / 2) + ":" + time);
		});
	}
	...
	barAnimationStyle() {
		...
		return {
			...
			//backgroundColor: "#cccccc", //here is the main slider
			height: "100%"
		};
	}
	...
	renderValue() {
		...
		const decimalStr = value.toString().split('.')[1];
		console.log(decimalStr / 1.66, 'valuevaluevaluevalue->')
		return (
			<Animated.View style={[
				...
				<Animated.View
					...
					<View style={[Style.valueInnerContainer, this.valueInnerContainerStyle, { height: 30, width: 30 }]}>
						<Animated.Text style={[Style.value, this.valueStyle, { color: "#30046B", fontWeight: "bold" }]} numberOfLines={1}>{`${parseInt(value / 2)}:${parseInt(decimalStr / 1.66)}`}</Animated.Text>
					</View>
				</Animated.View>
			</Animated.View>       
		);
		...
		return (
			<View>
				...
				<Animated.View style={[
					...
					{/* <Text style={[Style.min, this.minStyle, { color: "#30046B" }]}>{`${"1:00"}`}</Text>
					<Text style={[Style.max, this.maxStyle, { color: "#30046B" }]}>{`${"24:00"}`}</Text> */}
					{this.renderValue()}
					<View {...this.panResponder.panHandlers} style={[Style.touchableArea]} />
					<View style={{ borderRadius: 10, position: 'absolute', left: 0, right: 0, top: 33, bottom: 33, backgroundColor: "#cccccc", zIndex: -1 }}></View>
				</Animated.View>
			</View>
		);
	}
